...
module HKDF {
  ...
  method Extract(hmac: HMac, salt: seq<uint8>, ikm: seq<uint8>, ghost digest: Types.DigestAlgorithm) returns (prk: seq<uint8>)
   ...
   */
  method Hkdf(digest: Types.DigestAlgorithm, salt: Option<seq<uint8>>, ikm: seq<uint8>, info: seq<uint8>, L: int) returns (okm: seq<uint8>)
